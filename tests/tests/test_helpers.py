...
class StaticHashedStorageTestsMixin(object):
    ...
    def setUpClass(cls):
        ...
        content.close()
        content = File(open(os.path.join('tests', 'static', 'tests', 'images', 'dummy-static-svg.svg'), 'rb'))
        cls.svg_hash = hash_mixin.file_hash('tests/images/dummy-static-svg.svg', content)
        content.close()
