...
class SetCredentialsWithoutEnvVariablesTests(SimpleTestCase):
    ...
    def test_missing_API_SECRET_setting_raises_error(self):
        self.assert_incomplete_settings_raise_error(
            {'CLOUD_NAME': 'name', 'API_KEY': 'key'})

    def test_missing_API_KEY_setting_raises_error(self):
        self.assert_incomplete_settings_raise_error(
            {'CLOUD_NAME': 'name', 'API_SECRET': 'secret'})

    @mock.patch('cloudinary_storage.app_settings.cloudinary.config')
    def test_proper_configuration_correctly_sets_credentials(self,
                                                             config_mock):
        set_credentials(
            {'CLOUD_NAME': 'name', 'API_SECRET': 'secret', 'API_KEY': 'key'})
...
class SetCredentialsWithEnvVariablesTests(SimpleTestCase):
    ...
    @mock.patch.dict(os.environ, {'CLOUDINARY_API_SECRET': 'secret',
                                  'CLOUDINARY_API_KEY': 'key'}, clear=True)
    def test_missing_CLOUD_NAME_variable_raises_error(self):
        self.assert_incomplete_settings_raise_error()

    @mock.patch.dict(os.environ, {'CLOUDINARY_CLOUD_NAME': 'name',
                                  'CLOUDINARY_API_KEY': 'key'}, clear=True)
    ...
    @mock.patch.dict(os.environ,
                     ...
                     clear=True)
    def test_complete_set_of_env_variables_doesnt_raise_error(self,
                                                              config_mock):
        set_credentials({})
