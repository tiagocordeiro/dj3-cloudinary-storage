...
def set_credentials(user_settings):
    try:
        credentials = itemgetter('CLOUD_NAME', 'API_KEY', 'API_SECRET')(
            user_settings)
    except KeyError:
        if os.environ.get('CLOUDINARY_URL'):
            return
        if (os.environ.get('CLOUDINARY_CLOUD_NAME') and os.environ.get(
                'CLOUDINARY_API_KEY') and os.environ.get(
                'CLOUDINARY_API_SECRET')):
            return
        else:
            raise ImproperlyConfigured(
                'In order to use cloudinary storage, you need to provide '
...
INVALID_VIDEO_ERROR_MESSAGE = user_settings.get('INVALID_VIDEO_ERROR_MESSAGE',
                                                'Please upload a valid video file.')
EXCLUDE_DELETE_ORPHANED_MEDIA_PATHS = user_settings.get(
    'EXCLUDE_DELETE_ORPHANED_MEDIA_PATHS', ())

STATIC_TAG = user_settings.get('STATIC_TAG', 'static')
STATICFILES_MANIFEST_ROOT = user_settings.get('STATICFILES_MANIFEST_ROOT',
